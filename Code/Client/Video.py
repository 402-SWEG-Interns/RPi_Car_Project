...
class VideoStreaming:
    ...
    def face_detect(self,img,res,name):
...
#----------------------------------------------------------------------------------------------
            ...
            min_conf_threshold = 0.15 #original is 0.3

            self.face_x = 0 # resets face value, so if there is no detected object, it is set to nothing
            self.face_y = 0
            ...
            frame_rgb = cv2.cvtColor(res, cv2.COLOR_BGR2RGB) #img
            ...
            for i in range(len(scores)):
                ...
                if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0) and ((labels[int(classes[i])] == 'sports ball') or (labels[int(classes[i])] == 'apple') or (labels[int(classes[i])] == 'bowl') or (labels[int(classes[i])] == 'toilet'))):
                    ...
                    label = '%s: %d%%' % (name, int(scores[i]*100)) # Example: 'ballz: 72%'
    ...
    def color_ball_detect(self,img,color):
        try:
            ...
            hsc_dic = {'red': (np.array([136,87,111], np.uint8), np.array([180,255,255], np.uint8), 'red ballz'),
                       'blue': (np.array([90,150,150], np.uint8), np.array([110,255,255], np.uint8), 'blue ballz'),
                       'green': (np.array([36,70,40], np.uint8), np.array([86,255,255], np.uint8), 'green ballz'),
                       'yellow': (np.array([10,90,180], np.uint8), np.array([35,255,255], np.uint8), 'yellow ballz')}
            
            limits = hsc_dic[color.lower()]
            name = limits[2]
            self.lastSeen = limits[2]
            mask = cv2.inRange(hsvFrame, limits[0], limits[1])
            res = cv2.bitwise_and(img, img, mask = mask) 
            cv2.imwrite("res.jpg", res)
            self.face_detect(img, res, name)
